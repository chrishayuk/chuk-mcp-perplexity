...
help:
	...
	@echo "  check        - Run all CI checks (lint, typecheck, test-cov)"
...
publish:
	...
	if [[ $$REPLY =~ ^[Yy]$$ ]]; then \
		...
		echo "  https://github.com/chuk-mcp-servers/chuk-mcp-perplexity/actions"; \
	else \
		echo "Cancelled."; \
	fi
...
publish-test: build
	...
	if [[ $$REPLY =~ ^[Yy]$$ ]]; then \
		...
		echo "  pip install --index-url https://test.pypi.org/simple/ chuk-mcp-perplexity"; \
	else \
		echo "Cancelled."; \
	fi
